...
const AvailabilityPage = () => {
    ...
    return (
        <div className="p-4 md:p-8 bg-gray-100 min-h-screen">
            <h1 className="text-3xl font-bold text-gray-800 mb-8">Manage Availability & Leaves</h1>
            {error && <p className="text-red-500 bg-red-100 p-3 rounded-md mb-4">{error}</p>}

            {/* Weekly Schedule Section */}
            <section className="mb-10">
                <h2 className="text-2xl font-semibold text-gray-700 mb-4 flex items-center"><FaCalendarAlt className="mr-2 text-indigo-600"/>Weekly Schedule</h2>
                <form onSubmit={handleAddSlot} className="bg-white p-6 rounded-lg shadow-md mb-6 grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                    <div>
                        <label htmlFor="day_of_week" className="block text-sm font-medium text-gray-700">Day</label>
                        <select name="day_of_week" value={newSlot.day_of_week} onChange={handleSlotChange} className="mt-1 w-full p-2 border border-gray-300 rounded-md">
                            {daysOfWeek.map(day => <option key={day} value={day}>{day}</option>)}
                        </select>
                    </div>
                    <div>
                        <label htmlFor="start_time" className="block text-sm font-medium text-gray-700">Start Time</label>
                        <input type="time" name="start_time" value={newSlot.start_time} onChange={handleSlotChange} className="mt-1 w-full p-2 border border-gray-300 rounded-md"/>
                    </div>
                    <div>
                        <label htmlFor="end_time" className="block text-sm font-medium text-gray-700">End Time</label>
                        <input type="time" name="end_time" value={newSlot.end_time} onChange={handleSlotChange} className="mt-1 w-full p-2 border border-gray-300 rounded-md"/>
                    </div>
                    <button type="submit" className="bg-indigo-600 hover:bg-indigo-700 text-white font-semibold py-2 px-4 rounded-lg shadow-sm flex items-center justify-center h-10">
                        ...
                        availabilities.length > 0 ? (
                            <ul className="space-y-3">
                                {availabilities.map(slot => (
                                    <li key={slot.id} className="flex justify-between items-center p-3 bg-gray-50 rounded-md border">
            ...
            {/* Leaves Section */}
            <section id="leaves">
                <h2 className="text-2xl font-semibold text-gray-700 mb-4 flex items-center"><FaMoon className="mr-2 text-indigo-600"/>Manage Leaves</h2>
                 <form onSubmit={handleAddLeave} className="bg-white p-6 rounded-lg shadow-md mb-6 grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                    <div>
                        <label htmlFor="start_date" className="block text-sm font-medium text-gray-700">Start Date</label>
                        <input type="date" name="start_date" value={newLeave.start_date} onChange={handleLeaveChange} required className="mt-1 w-full p-2 border border-gray-300 rounded-md"/>
                    </div>
                    <div>
                        <label htmlFor="end_date" className="block text-sm font-medium text-gray-700">End Date</label>
                        <input type="date" name="end_date" value={newLeave.end_date} onChange={handleLeaveChange} required className="mt-1 w-full p-2 border border-gray-300 rounded-md"/>
                    </div>
                    <div className="md:col-span-1">
                        <label htmlFor="reason" className="block text-sm font-medium text-gray-700">Reason (Optional)</label>
                        <input type="text" name="reason" value={newLeave.reason} onChange={handleLeaveChange} className="mt-1 w-full p-2 border border-gray-300 rounded-md" placeholder="e.g., Vacation"/>
                    </div>
                    <button type="submit" className="bg-green-500 hover:bg-green-600 text-white font-semibold py-2 px-4 rounded-lg shadow-sm flex items-center justify-center h-10">
                 ...
                 <div className="bg-white shadow-xl rounded-lg p-6">
                    {loading.leaves ? <FaSpinner className="animate-spin h-6 w-6 text-indigo-500 mx-auto"/> : (
                        leaves.length > 0 ? (
                            <ul className="space-y-3">
                                {leaves.map(leave => (
                                    <li key={leave.id} className="flex justify-between items-center p-3 bg-red-50 rounded-md border border-red-200">
                                        <div>
